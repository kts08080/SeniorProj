...
uint8_t write_s_EEPROM(uint16_t addr, char *data) {
    ...
    // Send address + data to the EEPROM
    if (HAL_I2C_Master_Transmit(&EEPROM_PORT, EEPROM_I2C_ADDR, buffer, 3, I2C_COMM_TIMEOUT) != HAL_OK) {
        return HAL_ERROR;  // Error if transmission fails
    }

    // EEPROM needs some time to complete the write operation. Delay accordingly.
